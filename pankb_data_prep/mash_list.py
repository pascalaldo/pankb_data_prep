...
def initialize_parser(parser):
    parser.description = "Summarize MASH output."
    parser.add_argument(
        "--genomes",
        "-g",
        type=str,
...
def generate_mash_list(genomes, gtdb_meta_path, mash_file_path, mash_list_path):
    ...
    df_hclusts = pd.DataFrame(
        index=df_mash_corr.index, columns=["hcluster", "color_code"]
    )
...
def run(args):
    ...
    generate_mash_list(genomes, args.gtdb_meta, args.mash_file, args.output)
